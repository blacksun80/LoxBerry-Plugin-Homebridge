...
#
# Schritt 1: Config aus dem Backup zurueckholen.
# Schritt 2: Runtime aus dem Backup zurueckholen.
# Schritt 3: Benoetigte Node-Major-Version ermitteln.
# Schritt 4: Isoliertes Node.js einrichten.
# Schritt 5: Homebridge + Config UI X installieren (+ Rebuild bei Node-Wechsel).
...
# Kommando im Hintergrund ausfuehren, alle 15s eine Heartbeat-Zeile ausgeben.
run_with_heartbeat() {
    ...
    while kill -0 "$pid" 2>/dev/null; do
        ...
        last=$(tail -n 1 "$logfile" 2>/dev/null)
...
# Node-Version pruefen (Fehlerausgabe unterdrueckt).
CURRENT_LOCAL_VERSION=""
[ -x "$HB_NODE_DIR/bin/node" ] && CURRENT_LOCAL_VERSION=$("$HB_NODE_DIR/bin/node" -v 2>/dev/null)
...
# npm-global bleibt bei Node-Wechsel erhalten (enthaelt nachinstallierte Plugins).
...
if [ "$NODE_CHANGED" -eq 0 ] \
   ...
   && [ -n "$LATEST_HB_UI_VERSION" ] && [ "$INSTALLED_HB_UI_VERSION" = "$LATEST_HB_UI_VERSION" ]; then
    ...
    # Eigener npm-Cache in der Runtime, damit ein evtl. mit root-eigenen
    # Dateien vergifteter Shared-Cache umgangen wird.
    if ! run_with_heartbeat "npm install" "$NPM_INSTALL_LOG" \
            "$LOCAL_NPM" install -g --prefix "$HB_NPM_GLOBAL" \
            --cache "$NPM_CACHE_DIR" --no-audit --no-fund --loglevel=http \
            homebridge homebridge-config-ui-x; then
    ...
    if [ "$NODE_CHANGED" -eq 1 ] && [ -n "$EXTRA_PKGS" ]; then
        ...
        if run_with_heartbeat "Rebuild" "$NPM_INSTALL_LOG" \
            ...
            if run_with_heartbeat "npm rebuild" "$NPM_INSTALL_LOG" \
                    "$LOCAL_NPM" rebuild -g --prefix "$HB_NPM_GLOBAL" --cache "$NPM_CACHE_DIR"; then
...
# Alles bis hier lief als root; der Dienst laeuft als "loxberry" (Schritt 6) -
# deshalb jetzt zurueckchownen. Kein sudo noetig, wir sind schon root.
